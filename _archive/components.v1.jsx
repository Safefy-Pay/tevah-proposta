...
/* Studio product placeholder. Drop real product photography here later. */
const Photo = ({ ratio = "3/4", tag = "PRODUCT SHOT", tone = "light", children, className = "", style = {} }) => {
  ...
  return (
    <div className={"tv-photo " + className} style={{ aspectRatio: ratio, ...toneStyle, ...style }}>
      <span className="tag" style={ tone !== "light" ? { color: "rgba(255,255,255,.55)" } : {}}>{tag}</span>
      {children}
    </div>
  );
};
    ...
    <header className="nav-glass sticky top-0 z-40">
      <div className="max-w-[1440px] mx-auto px-6 lg:px-10 h-[68px] flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <button className="h-10 px-3 rounded-full hover:bg-black/[.04] transition flex items-center gap-2" aria-label="Sacola">
            <Icon name="bag"/>
            <span className="text-[12px] font-mono">2</span>
          </button>
        </div>
      </div>
  ...
  return (
    <a href="#" className="product-card group block">
      <div className="relative">
        <Photo ratio="3/4" tone={p.toneA || "light"} tag={p.tagA || "FRONT VIEW"} className="img-a absolute inset-0"/>
        <Photo ratio="3/4" tone={p.toneB || "warm"} tag={p.tagB || "ALT VIEW"} className="img-b absolute inset-0"/>
        ...
        <div className="absolute inset-x-3 bottom-3 opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition-all duration-500">
          <div className="bg-white/95 backdrop-blur rounded-lg border border-[var(--tv-line)] flex">
            <button className="flex-1 h-10 text-[12px] font-medium hover:bg-black/[.04]">Vista rápida</button>
            <span className="w-px bg-[var(--tv-line)]"/>
            <button className="flex-1 h-10 text-[12px] font-medium hover:bg-black/[.04]">+ Sacola</button>
          </div>
        </div>
      </div>
